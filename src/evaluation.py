...
def calculate_metrics(y_test: pd.Series, y_pred: pd.Series, y_pred_proba: pd.Series | None) -> dict[str, float]:
    metrics = {
...
def log_to_mlflow(trained_model: Pipeline, metrics: dict[str, float], x_train: pd.DataFrame, x_test: pd.DataFrame) -> None:
    import mlflow
...
def evaluate_predictions(
    ...
    validate_evaluation_inputs(x_train, x_test, y_train, y_test, y_pred)
    metrics = calculate_metrics(y_test, y_pred, y_pred_proba)
    report = classification_report(y_test, y_pred, output_dict=True, zero_division=0)
    prediction_results = create_prediction_results(x_test, y_test, y_pred, y_pred_proba)

    if trained_model is not None:
        log_to_mlflow(trained_model, metrics, x_train, x_test)
...
def evaluate_model(
    ...
    return evaluate_predictions(x_train, x_test, y_train, y_test, y_pred, y_pred_proba, trained_model)
